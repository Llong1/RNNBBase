...
export var createNBNetworkApi = function (method, cHeaders, c) {
    return HeaderManager.getHeaders().then(function (h) {
        var headers = __assign(__assign({}, h), cHeaders);
        var conf = __assign({ baseURL: Constants.BaseDomain, method: method || 'GET', transformResponse: function (data) {
                return data;
            }, headers: headers }, c);
        ...
        if (method) {
            switch (method) {
                ...
                case 'POST':
                    return a.post(api, params);
                default:
                    return a({ url: api, params: params });
            }
        }
        else {
